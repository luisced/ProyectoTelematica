...
example_table.add_column(from_markup(
...
**Welcome**! This tool will serve for configuring disco devices.
"""
...
Cisco devices are integral components of networking infrastructure.

Here are some of the key Cisco devices:
...
- **Switch** - A device in a computer network that connects other devices together. Multiple data cables are plugged into a switch to enable communication between different networked devices.
...
class Welcome(Container):
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        self.app.query_one(
            ".location-first").scroll_visible(duration=0.5, top=True)
...
class DemoApp(App[None]):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Container(
            ...
            RichLog(classes="-hidden", wrap=False,
                    highlight=True, markup=True),
            Body(
                QuickAccess(
                    LocationLink("Config files", ".location-top"),
                    LocationLink("Devices", ".location-widgets"),
                    LocationLink("Topology", ".location-rich"),
                ),
                ...
                Column(
                    Section(
                        ...
                        Static(Pretty(DATA, indent_guides=True),
                               classes="pretty pad"),
                        SubTitle("JSON"),
                        Window(Static(JSON(JSON_EXAMPLE),
                               expand=True), classes="pad"),
                        SubTitle("Tables"),
